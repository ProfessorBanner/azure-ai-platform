# Production (prod) platform root (state key: platform/prod.tfstate).
#
# PROD is the final governed deployment environment in the dev -> stg -> prod
# promotion chain; its apply is gated by a manual approval. It composes the SAME
# reusable modules as every other environment (no forks): storage, Key Vault,
# Log Analytics, a Premium Databricks workspace, and a Databricks Access
# Connector, into the externally provisioned rg-aiplatform-prod (referenced by
# data source, never created or destroyed).
...
# --- Phase 13.3 PROD storage private connectivity ---------------------------
...
# --- Phase 13.3 Unity Catalog storage foundation ----------------------------
...
# --- Phase 13.4 PROD Unity Catalog foundation ------------------------------

resource "databricks_storage_credential" "prod" {
  name = "sc-aiplatform-prod"
  ...
  comment = "PROD Unity Catalog storage credential using the PROD Access Connector managed identity."
}

resource "databricks_external_location" "prod" {
  name            = "el-aiplatform-prod"
  url             = "abfss://ucroot@stexampleproddata.dfs.core.windows.net/"
  credential_name = databricks_storage_credential.prod.name
  isolation_mode  = "ISOLATION_MODE_ISOLATED"

  comment = "PROD Unity Catalog external location backed by private platform ADLS."
}

resource "databricks_catalog" "prod" {
  name           = "prod"
  storage_root   = "abfss://ucroot@stexampleproddata.dfs.core.windows.net/"
  isolation_mode = "ISOLATED"

  comment = "Terraform-managed PROD catalog backed by platform ADLS."

  depends_on = [
    databricks_external_location.prod
  ]
}
...
resource "databricks_schema" "platform_test" {
  ...
  comment = "PROD platform integration validation schema."
}

# --- Phase 13.4 PROD Unity Catalog workspace bindings ----------------------
...
resource "databricks_workspace_binding" "prod_external_location" {
  securable_name = databricks_external_location.prod.name
  securable_type = "external_location"
  workspace_id   = tonumber(module.databricks_workspace.databricks_workspace_numeric_id)
  binding_type   = "BINDING_TYPE_READ_WRITE"
}

resource "databricks_workspace_binding" "prod_storage_credential" {
  securable_name = databricks_storage_credential.prod.name
...
#
# First consumer of the reusable product-UC primitive: prod.hello_databricks,
# a product-owned schema inside the platform-owned PROD catalog. The catalog,
...
locals {
  ...
  hello_databricks_runtime_principal = (
    local.hello_databricks_bundle.targets.prod.run_as.service_principal_name
  )
}
...
module "hello_databricks_uc" {
  ...
  comment = "hello-databricks product schema (PROD). Managed by Terraform; tables inside are product-owned."
}

# --- Phase 15 ml-lifecycle-demo product Unity Catalog -----------------------
#
# Second consumer of the reusable product-UC primitive: prod.ml_lifecycle_demo,
# a product-owned schema inside the platform-owned PROD catalog. The catalog,
...
locals {
  ...
  ml_lifecycle_demo_runtime_principal = (
    local.ml_lifecycle_demo_bundle.targets.prod.run_as.service_principal_name
  )
}
...
module "ml_lifecycle_demo_uc" {
  ...
  comment = "ml-lifecycle-demo product schema (PROD). Managed by Terraform; tables and registered models inside are product-owned."
}
